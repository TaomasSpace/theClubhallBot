...
def setup(bot: commands.Bot):
    ...
    async def setstatpoints(interaction: discord.Interaction, user: discord.Member, amount: int):
        ...
        await interaction.response.send_message(f"\u2705 Set {user.display_name}'s stat points to {amount}.", ephemeral=True)

    @bot.tree.command(name="lastdate", description="Get a last date of user (Admin/Owner only)")
    @app_commands.describe(user="User")
    async def lastdate(interaction: discord.Interaction, user: discord.Member):
        if not has_role(interaction.user, ADMIN_ROLE_ID) and not interaction.user.premium_since:
    ...
    @app_commands.describe(user="Target user", stat="Which stat to set (intelligence, strength, stealth)", amount="New stat value (\u2265 0)")
    async def setstat(interaction: discord.Interaction, user: discord.Member, stat: str, amount: int):
        if not has_role(interaction.user, ADMIN_ROLE_ID):
            await interaction.response.send_message("Only the Owner can use this command.", ephemeral=True)
            return
        ...
        await interaction.response.send_message(f"\u2705 Set {user.display_name}'s **{stat}** to **{amount}**.", ephemeral=True)

    @bot.tree.command(name="addshoprole", description="Create/register a purchasable role (Owner/Admin)")
    @app_commands.describe(name="Role name", price="Cost in coins", color="#RRGGBB (hex)", reference="Put the new role relative to this role (optional)", above="If True, place *above* the reference; else below")
    async def addshoprole(inter: discord.Interaction, name: str, price: int, color: str, reference: discord.Role | None = None, above: bool = True):
        ...
        if reference:
            ...
            try:
                ...
                await inter.response.send_message("\u274c Bot kann die Rolle nicht verschieben. Achte darauf, dass seine h\xf6chste Rolle \xfcber dem Ziel steht.", ephemeral=True)
                return
        add_shop_role(role.id, price)
        await inter.response.send_message(f"\u2705 Rolle **{role.name}** registriert (Preis {price} Coins).", ephemeral=True)
    ...
    async def shop(inter: discord.Interaction):
        ...
        embed = discord.Embed(title="\U0001f6d2 Role Shop", description="\n".join(lines))
    ...
    async def buyrole(inter: discord.Interaction, role: discord.Role):
        ...
        await inter.response.send_message(f"\U0001f389 Congratulation! You bought **{role.name}** for {price} clubhall coins.")

    @bot.tree.command(name="manageprisonmember", description="Send or free someone from prison")
    @app_commands.describe(user="Person you want to lock or free in prison", time="Duration (e.g. 10m, 2h) or 'cancel'")
    async def managePrisonMember(interaction: discord.Interaction, user: discord.Member, time: str | None = None):
        if not has_role(interaction.user, ADMIN_ROLE_ID):
        ...
        if time == "cancel":
            ...
            if task and not task.done():
                ...
                await interaction.response.send_message(f"\U0001f54a\ufe0f Timer cancelled. {user.mention} has been freed.", ephemeral=gooyb)
            else:
                await interaction.response.send_message(f"\u26a0\ufe0f No active timer for {user.mention}.", ephemeral=True)
            return
        if role not in user.roles:
            ...
            await interaction.response.send_message(f"\U0001f513 {user.mention} has been freed from prison.", ephemeral=gooyb)
            return
        await user.remove_roles(role)
        ...
        if time:
            ...
            if seconds is None:
                await interaction.response.send_message(f"\u23f3 Invalid time format: `{time}`. Use `10m`, `2h`, etc.", ephemeral=True)
                return
            async def release_later():
                try:
                    await asyncio.sleep(seconds)
                    await user.add_roles(role)
                    await interaction.followup.send(f"\U0001f54a\ufe0f {user.mention} has served their time and is now free.", ephemeral=False)
    ...
    @bot.tree.command(name="addcolorreactionrole", description="Add emoji-role to color reaction message")
    @app_commands.describe(emoji="Emoji to react with", role="Role to assign")
    async def addcolorreactionrole(interaction: discord.Interaction, target_message_id: int, emoji: str, role: discord.Role):
        if not has_role(interaction.user, ADMIN_ROLE_ID):
            await interaction.response.send_message("No permission.", ephemeral=True)
            return
        ...
        try:
            ...
            _execute = __import__('db.DBHelper', fromlist=['_execute'])._execute
            _execute("INSERT OR REPLACE INTO reaction_roles (message_id, emoji, role_id) VALUES (?, ?, ?)", (str(target_message_id), emoji, str(role.id)))
            await interaction.response.send_message(f"\u2705 Added emoji {emoji} for role {role.name}.", ephemeral=True)
    ...
    async def imitate(interaction: discord.Interaction, user: discord.Member, msg: str):
        ...
        try:
            await webhook.send(content=msg, username=user.display_name, avatar_url=user.display_avatar.url, allowed_mentions=discord.AllowedMentions.none())
    ...
    async def giveaway(interaction: discord.Interaction, duration: int, prize: str, winners: int):
        if not has_role(interaction.user, ADMIN_ROLE_ID) and not interaction.user.id == 875279455222898698:
            await interaction.response.send_message("Only admins and owners can use this command", ephemeral=True)
            return
        if winners < 1:
            await interaction.response.send_message("You need at least 1 winner.", ephemeral=True)
            return
        embed = discord.Embed(title="\U0001f389 GIVEAWAY \U0001f389", description=f"React with \U0001f389 to win **{prize}**!\n\U0001f514 Duration: **{duration} min**.\n\U0001f3c6 Winners: **{winners}**", color=discord.Color.gold())
        ...
        create_giveaway(str(giveaway_msg.id), str(giveaway_msg.channel.id), end_time, prize, winners)
        from events import end_giveaway, active_giveaway_tasks
        async def end_later():
            try:
                await asyncio.sleep(duration * 60)
                await end_giveaway(bot, giveaway_msg.channel.id, giveaway_msg.id, prize, winners)
            except asyncio.CancelledError:
                pass
        task = asyncio.create_task(end_later())
        active_giveaway_tasks[giveaway_msg.id] = task

    @bot.tree.command(name="createrole", description="Create a role and assign to users (for goodyb & nannapat2410 only)")
    @app_commands.describe(role_name="Name of the role", role_color="Hex color like #000000", member1="1st member", member2="2nd member", member3="3rd member", member4="4th member", member5="5th member")
    async def createrole(interaction: discord.Interaction, role_name: str, role_color: str, member1: discord.Member, member2: discord.Member | None = None, member3: discord.Member | None = None, member4: discord.Member | None = None, member5: discord.Member | None = None):
        ...
        try:
            ...
            await interaction.response.send_message("\u26a0\ufe0f Invalid hex color.", ephemeral=True)
            return
        try:
            role = await interaction.guild.create_role(name=role_name, colour=colour_obj)
        except discord.Forbidden:
            await interaction.response.send_message("\u274c I can't create roles.", ephemeral=True)
            return
