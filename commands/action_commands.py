...
def setup(bot: commands.Bot):
    ...
    async def steal(interaction: discord.Interaction, target: discord.Member):
        ...
        if cooldown and now - cooldown < timedelta(minutes=45):
            ...
            await interaction.response.send_message(f"\u23f3 You can steal again in **{minutes} minutes {seconds} seconds**.", ephemeral=True)
            return
        ...
        if random() > success_chance:
            await interaction.response.send_message("\U0001f440 You were caught and failed to steal any coins!", ephemeral=True)
            return
        ...
        await interaction.response.send_message(f"\U0001f576\ufe0f Success! You stole **{stolen_amt}** coins from {target.display_name}.", ephemeral=True)
    ...
    async def hack(interaction: discord.Interaction):
        ...
        if cooldown and now - cooldown < timedelta(minutes=45):
            remaining = timedelta(minutes=45) - (now - cooldown)
            minutes, seconds = divmod(int(remaining.total_seconds()), 60)
            await interaction.response.send_message(f"\u23f3 You can hack again in **{minutes} minutes {seconds} seconds**.", ephemeral=True)
            return
        stats = get_stats(uid)
        if stats["intelligence"] < 3:
            await interaction.response.send_message("\u274c You need at least **3** Intelligence to attempt a hack.", ephemeral=True)
            return
        ...
        if not success:
            ...
            await interaction.response.send_message(f"\U0001f4bb Hack failed! Security traced you and you lost **{loss}** coins.", ephemeral=True)
            return
        reward = randint(5, 12) * int_level
        added = safe_add_coins(uid, reward)
        if added > 0:
            await interaction.response.send_message(f"\U0001f50b Hack successful! You siphoned **{added}** coins from the bank.", ephemeral=True)
        else:
            await interaction.response.send_message("\u26a0\ufe0f Hack succeeded but server coin limit reached. No coins added.", ephemeral=True)
    ...
    async def fight(interaction: discord.Interaction, target: discord.Member):
        ...
        if cooldown and now - cooldown < timedelta(minutes=45):
            remaining = timedelta(minutes=45) - (now - cooldown)
            minutes, seconds = divmod(int(remaining.total_seconds()), 60)
            await interaction.response.send_message(f"\u23f3 You can fight again in **{minutes} minutes {seconds} seconds**.", ephemeral=True)
            return
        register_user(uid, interaction.user.display_name)
        register_user(tid, target.display_name)
        atk = get_stats(uid)
        ...
        if random() > win_chance:
            ...
            await interaction.response.send_message(f"\U0001f3cb\ufe0f You lost the fight and paid **{penalty}** coins in damages to {target.display_name}.", ephemeral=True)
            return
        ...
        await interaction.response.send_message(f"\U0001f4aa Victory! You took **{stolen}** coins from {target.display_name}.", ephemeral=True)
